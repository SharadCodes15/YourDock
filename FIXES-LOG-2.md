...
## 8. FIX: Black Screen / Electron Default Screen on Startup
- **Root Cause**: The `app.whenReady().then(async () => { ... })` startup promise chain in [main.js](file:///e:/sideProjects/dock/main.js) had no `.catch()` handler and no try/catch around the window-creation sequence. If any async operation inside threw (e.g. `loadConfig()`, `loadSettings()`, `startNormalApp()`), the error was silently swallowed as an unhandled promise rejection, no window was ever created, and Electron fell back to its built-in default screen (showing "Electron v31.7.7" with Blog/Repository/Docs links).
- **Fix**: Wrapped the entire `app.whenReady()` startup body in a try/catch block that logs the error, writes to `logs/error.log`, records a crash report, shows a `dialog.showErrorBox()` with the actual error message, and calls `app.quit()`. Added a `.catch()` on the outer promise chain as a secondary safety net. This ensures startup failures are always surfaced visibly instead of silently falling back to Electron's default screen.
- **Verification**: `npm start` launches with `[startup] app ready`, Menu Bar and Dock windows appear correctly, no console errors, no Electron default screen.
